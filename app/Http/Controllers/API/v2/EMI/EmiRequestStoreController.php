...
namespace App\Http\Controllers\API\v2\EMI;

use App\Http\Controllers\Controller;
use App\Http\Requests\API\v2\EmiApplyCardRequest;
use App\Http\Requests\API\v2\EmiWithCitizenshipRequest;
use App\Http\Requests\API\v2\EmiWithCreditCardRequest;
...
class EmiRequestStoreController extends Controller
{
    ...
    private function storeCreditCard(Request $request, Product $product)
    {
        ...
        try {
            ...
            if (!empty($validated['signature']) && is_string($validated['signature'])) {
                $this->attachExistingFilesByKey(
                    usageType: $emiRequest->getTable(),
                    usageId: $emiRequest->id,
                    documents: ['signature' => $validated['signature']],
    ...
    private function storeCitizenship(Request $request, Product $product)
    {
        ...
        try {
            ...
            $this->attachExistingFilesByKey(
                ...
                documents: $validated['guarantor']['documents'] ?? [],
                defaultTitle: 'guarantor_document'
            );
    ...
    /**
     * Attach existing uploaded files (by files.key UUID) to a usage target via file_usages.
     *
     * Supports both:
     * - list: ['uuid1','uuid2'] -> titles become "{$defaultTitle}_0", "{$defaultTitle}_1"
     * - map:  ['citizenship_front' => 'uuid', ...] -> titles become the map keys
     *
     * @param array<int|string, mixed> $documents
     */
    private function attachExistingFilesByKey(string $usageType, int|string $usageId, array $documents, string $defaultTitle): void
    {
        foreach ($documents as $k => $v) {
            ...
            $fileKey = trim($v);
            $file = FileModel::where('key', $fileKey)->first();
            if (!$file) {
                continue;
            }

            $title = is_string($k) ? $k : ($defaultTitle . '_' . (string) $k);

            $exists = FileUsageModel::where('file_id', $file->id)
